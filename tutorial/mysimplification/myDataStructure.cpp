...
myDataStructure::myDataStructure(const Eigen::MatrixXi& F, const Eigen::MatrixXd& V) : F(F), V(V) {
    ...
    Q = {};
    C.resize(E.rows(), V.cols());//Here we initailize the C matrix to have the number of edges as rows and 3 columns 
...
bool myDataStructure::simplify(int faces) {
...
myDataStructure::myDataStructure(const Eigen::MatrixXi& F, const Eigen::MatrixXd& V, int x) : F(F), V(V) {
    edge_flaps(F, this->E, this->EMAP, this->EF, this->EI);
    ////Here we want to fill the priority queue
    myQ.clear();
