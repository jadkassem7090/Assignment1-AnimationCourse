...
int main()
{
    std::thread mySong([]() {PlaySound(TEXT("data/song.wav"), NULL, SND_FILENAME | SND_ASYNC | SND_LOOP); });
    const int DISPLAY_WIDTH = 800;
    const int DISPLAY_HEIGHT = 800;
    const float CAMERA_ANGLE = 45.0f;
    const float NEARR = 0.1f;
    const float FARR = 120.0f;

    Renderer renderer;
    Display display("LastProject", DISPLAY_WIDTH, DISPLAY_HEIGHT, &renderer);
    ...
    scene->Init(CAMERA_ANGLE, DISPLAY_WIDTH, DISPLAY_HEIGHT, NEARR, FARR);
